...
I set the threshold, or target density, at 0.02 as suggested in the human cells \citep{Bodor2014}. In mean-field approximation, similar to the basic model, low $\alpha$ values had only one fixed point at $\rho$=0 while it became unstable as $\alpha$ increased and a new non-zero stable fixed point emerged(Figure~\ref{fig:numberControl}A). The bifurcation diagram also indicated that it is a transcritical bifurcation (Figure~\ref{fig:numberControl}B). However, differently, $\rho$ of the fixed point was maintained at the target density 0.02, independent of $\alpha$. 

To implement arbitrary number control in simulation, I set the replenishment strength to be negatively correlated with the number of CENP-A nucleosomes. To this end, I explored two different methods, either changing $\alpha$ or changing $rr$ according to density, and both of them gave similar results. The density evolution map indicated that arbitrary number control successfully stabilised CENP-A density around the target 0.02. Interestingly, from the kymograph, it can be seen that although multiple islands existed in the first few time steps, only very few, a single one for most of the time, were maintained throughout the simulation (Figure~\ref{fig:numberControl}C and D). This result is consistent with the established conclusion from the cell polarity determination research that auto-catalytic reaction and mass conservation lead to the formation of a single maximum \citep{Goryachev2020CompeteOutcomes, Goehring2012OrganelleComponents, Otsuji2007APolarity}. Nevertheless, I concluded that arbitrary number control was insufficient to explain the dynamics of the CENP-A system. 

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{chapter2/figures/number_conrol.pdf}
  \caption[Arbitrary number control resulted in a pattern of one or two large islands]{Arbitrary number control resulted in a pattern of one or two large islands. (A) Cobweb plot of the arbitrary number control model. The mean-field approximations of the arbitrary number control model with different $\alpha$s were shown as coloured curves. The fixed points were indicated as black dots. The bottom plot is the zoom-in of $\rho{t}$=0-0.1 and $\rho{t+1}$=0-0.1. (B) Bifurcation diagram of the arbitrary number control model. The fixed densities were plotted as a function of $\alpha$. The stable branch was shown as the solid line, whereas the unstable one was shown as the dotted line. (C) The density evolution map and spatial pattern kymographs of the arbitrary number control model. The simulations were conducted at $NN$=1000, $\alpha$=1, $\sigma$=3, $rr$=3, $\rho_{0}$=0.02, with a target density of 0.02. (D) The average number of islands of 10 simulations running for 2000 time steps. The error bar represents the standard deviation.}
...
\begin{table}[htbp]
\centering
\caption{Summary of the models tested}
\label{tab:summary}
\begin{tabular}{lccc}
\hline
\multicolumn{1}{c}{\textbf{Model}}                        & \textbf{Bi-stability}   & \textbf{Low density} & \textbf{Island pattern} \\ \hline
Basic                                                     & \ding{55} &          \ding{55}            &         \ding{55}                \\
Global   cooperativity                                    &  \ding{51}                       &        \ding{55}              &           \ding{55}              \\
Number   control                                          &          \ding{55}               &           \ding{51}           &      \ding{55}                   \\
Spontaneous   conversion                                  &          \ding{55}               &          \ding{51}            &    \ding{51}                     \\
\begin{tabular}[c]{@{}l@{}}Threshold-base stabilisation \\ + spontaneous conversion\end{tabular} &           \ding{51}              &           \ding{51}           &        \ding{51}                 \\ \hline
\end{tabular}
\end{table}

Summary table!
Any weak non-local mechanism will do
Important to test KT forming threshold and how does it compare to the level of noise
Alternative direction -- 3D model
