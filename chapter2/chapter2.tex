...
Therefore, on the second attempt, I implemented global cooperativity that only one of the two pre-existing CENP-A nucleosomes needed to be within the neighbourhood while the other one could be at any position. As expected, the global cooperative model was able to exhibit bi-stability. With the same parameter set, the asymptotic density of the model could be either 0 or at a high value, depending on the $\rho_{0}$. Consistent with the mean-field approximation, the asymptotic density of the 'upper state' could only be at values around 0.4, which is much higher than what is observed in biological systems \citep{Bodor2014}. The spatial distribution of CENP-A nucleosomes was also not comparable. Thus, I concluded that a 1D epigenetics model with cooperative deposition cannot recapitulate the CENP-A system we aimed to model. 
...
The model can be further used to study other questions in centromere biology. Recent human CENP-A ChIP-seq indicated a rather uniform distribution of CENP-A enrichment at the core centromere \citep{Logsdon2021The8}, which appeared to challenge the conclusion from extended chromatin experiments that CENP-A nucleosomes form an island pattern \citep{Blower2002ConservedHumans, Dunleavy2011H3.3Phase., Kyriacou2018}. However, it has to be noted that a ChIP-seq profile represents the average of the population and hence cannot be used to interpret the status of a single cell. Our model provides an opportunity to test whether the result from ChIP-seq is compatible with that from extended chromatin. The average spatial pattern of a large number of individually carried out simulations could be used to compare with the ChIP-seq profile. The finding that CENP-A is enriched at varied locations of satellite-free centromeres in different horse individuals suggests a potential sliding of CENP-A domains \citep{Nergadze2018BirthDomains}. Interestingly, it was observed in the simulation of our model that CENP-A islands could change their locations over time in a smooth manner (Figure~\ref{fig:modelBehaviour} and Figure~\ref{fig:numberControl}), which might provide support for the hypothesis. Developing tools to trace and quantify the sliding of CENP-A islands could add further insights. For easier computation and interpretation, we used several simplified assumptions that might not necessarily reflect the physics of the biological system. It would be informational to adjust those assumptions and test their effects on the model behaviour. Periodic boundary conditions were used in our model. Heterochromatinic peri-centromeres have been reported to limit the size of the CENP-A enriched core \citep{Scott2006ADomains, Sullivan2016HumanDeletion}, which suggests alternative boundary conditions other than periodic. Assessing the effect of different boundary conditions might be of interest. Similarly, in the model, we considered an array of continuously placed, equally spaced nucleosomes. However, the existence of centromeric transcription \citep{Arunkumar2020CentromericKnife} suggests a more sophisticated and dynamic distribution of nucleosomes, which might be worth exploring. Practically, an additional 'empty' state could be added to the CA cells besides the 'CENP-A' and 'H3' states. 